<template>
  <div>
    <h2 id="page-heading" data-cy="StatusHeading">
      <span v-text="$t('maintenanceApp.status.home.title')" id="status-heading">Statuses</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('maintenanceApp.status.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'StatusCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('maintenanceApp.status.home.createLabel')"> Create a new Status </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && statuses && statuses.length === 0">
      <span v-text="$t('maintenanceApp.status.home.notFound')">No statuses found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="statuses">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('maintenanceApp.status.statusName')">Status Name</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="maintenanceApp.status.delete.question" data-cy="statusDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-status-heading" v-text="$t('maintenanceApp.status.delete.question', { id: removeId })">
