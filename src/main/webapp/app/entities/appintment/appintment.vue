<template>
  <div>
    <h2 id="page-heading" data-cy="AppintmentHeading">
      <span v-text="$t('maintenanceApp.appintment.home.title')" id="appintment-heading">Appintments</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('maintenanceApp.appintment.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'AppintmentCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('maintenanceApp.appintment.home.createLabel')"> Create a new Appintment </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && appintments && appintments.length === 0">
      <span v-text="$t('maintenanceApp.appintment.home.notFound')">No appintments found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="appintments">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('maintenanceApp.appintment.appDate')">App Date</span></th>
            <th scope="row"><span v-text="$t('maintenanceApp.appintment.notes')">Notes</span></th>
            <th scope="row"><span v-text="$t('maintenanceApp.appintment.client')">Client</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="maintenanceApp.appintment.delete.question" data-cy="appintmentDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-appintment-heading" v-text="$t('maintenanceApp.appintment.delete.question', { id: removeId })">
